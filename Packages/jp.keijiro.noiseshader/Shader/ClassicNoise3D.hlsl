...
float cnoise(float3 P)
{
  ...
  float4 norm0 = wglnoise_taylorInvSqrt
    (float4(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
  g000 *= norm0.x;
  g010 *= norm0.y;
  g100 *= norm0.z;
  g110 *= norm0.w;

  float4 norm1 = wglnoise_taylorInvSqrt
    (float4(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
  g001 *= norm1.x;
  g011 *= norm1.y;
  g101 *= norm1.z;
  g111 *= norm1.w;
...
float pnoise(float3 P, float3 rep)
{
  ...
  float3 Pi1 = wglnoise_mod(Pi0 + (float3)1.0, rep); // Integer part + 1, mod period
  Pi0 = wglnoise_mod289(Pi0);
  Pi1 = wglnoise_mod289(Pi1);
  float3 Pf0 = frac(P); // Fractional part for interpolation
  float3 Pf1 = Pf0 - (float3)1.0; // Fractional part - 1.0
